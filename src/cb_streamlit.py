...
if file is not None:
    ...
    with open("danzzzlll/digital_break/main/clf_model.pkl", 'rb') as file:
...
# Apply the function to each row to compute the 'final type' column
    ...
    sub = pd.read_csv("danzzzlll/digital_break/main/data/submission.csv")
    ...
    if file_2 is not None:
        ...
        with open("danzzzlll/digital_break/main/models/file.pkl", 'rb') as file:
            X_2d = pickle.load(file)

        with open("danzzzlll/digital_break/main/models/labels.pkl", 'rb') as file:
